...
class vehicle{
  ...
  public:
    ...
    vehicle(string name){
      ...
      this->name = name;
      avg_speed = 5 + rand() % 91;
      avg_consumpt = avg_speed / 100f;
      current_fuel = 5 + rand() % 46;
      road_left = 25 + rand() % 200;
...
bagażnik reprezentowany przez dynamicznie zaalokowaną tablicę intów. Rozmiar tablicy jest równy rozmiarowi bagażnika z poprzedniego punktu zadania.
Każde pole tej tablicy ustaw na losową wartość z zakresu [0; 10]. Każde pole oznacza wagę jednego przedmiotu w bagażniku. 0 oznacza brak przedmiotu w danym miejscu.
...
Utwórz funkcję (ale nie metodę klasy!) która przyjmie za argumenty dwa wskaźniki na obiekty klasy samochód i zwróci nazwę samochodu, który jako pierwszy dojedzie do celu. W przypadku remisu zwróć słowo "---REMIS---".
